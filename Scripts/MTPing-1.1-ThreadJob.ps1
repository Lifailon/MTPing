...
$network = $textbox_network.text -replace "\s{1,3}" # удаляем пробелы из маски
$network = $network -replace "\.\d{1,3}$","." # удаляем 4-й октет

foreach ($4 in 1..254) {
$ip = $network+$4 # добавляем 4-й октет
...
# проверяем на выполнение (задания выполняются по очереди сверху вниз):
...
if ($ping -match "ttl") { # если в выводе есть ttl
$row_ip = $ping -replace " : .+","" # забираем ip, удаляя все после " : "
if ($CheckBox_DNS.Checked -eq $True) { # если включен CheckBox
# забираем третью строку, из нее удаляем все до ":" и все пробелы после:
$rdns = ((nslookup $row_ip)[3]) -replace ".{1,20}:\s{1,20}"
}
# удаляем TTL и его значение, из полученного результата находим единственное "=" 
# и удаляем значения до него включительно:
$time = ($ping -replace " TTL=.+","") -replace ".+(?<==)"
$DataGridView.Rows.Add("$row_ip","$rdns","Available",$time) # если есть ttl, пишем Available
$DataGridView.Rows[-2].Cells[2].Style.BackColor = "lightgreen" # красим
$on = $on +1 # добавляем 1 активный хост для статуса
} else {
$row_ip = $ping -replace " : .+",""
